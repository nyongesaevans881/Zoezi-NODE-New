...
const generateToken = async (req, res, next) => {
    const secretKey = process.env.PLAYBOX_MPESA_CONSUMER_SECRET;
    const consumerKey = process.env.PLAYBOX_MPESA_CONSUMER_KEY;
    const auth = Buffer.from(`${consumerKey}:${secretKey}`).toString("base64");

    const environmentAuthUrl = process.env.PLAYBOX_MPESA_AUTH_URL;
